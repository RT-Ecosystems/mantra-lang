...
// Extended script detection utilities
std::string ScriptDetector::getScriptName(Script script) {
    switch (script) {
        case Script::UNKNOWN: return "Unknown";
        case Script::ROMAN: return "Roman";
...
// Helper to detect script categories
bool isIndianScript(Script script) {
    return script != Script::ROMAN && script != Script::UNKNOWN && script != Script::MIXED;
}

// Extended detection for debugging and analysis
std::string getDetailedScriptInfo(const std::string& text) {
    std::string info = "Script Analysis Report:\n";
    info += "=====================\n";
    
    auto script_counts = ScriptDetector::countScripts(text);
    ...
    for (const auto& pair : script_counts) {
        double percentage = (total > 0) ? (static_cast<double>(pair.second) / total * 100.0) : 0.0;
        info += ScriptDetector::getScriptName(pair.first) + ": " + 
                std::to_string(pair.second) + " chars (" + 
                std::to_string(static_cast<int>(percentage)) + "%)\n";
    }
    
    info += "\nDominant Script: " + ScriptDetector::getScriptName(ScriptDetector::getDominantScript(text)) + "\n";
    info += "Mixed Script: " + std::string(ScriptDetector::isMixedScript(text) ? "Yes" : "No") + "\n";
    info += "Confidence: " + std::to_string(static_cast<int>(ScriptDetector::getDetectionConfidence(text) * 100)) + "%\n";
